...
export run_name="demo_vit_wonorm_R2loss_newsplits"
export yaml_config=./config/Demo_graph_vit_pm2.5_newsplits.yaml

export NN=1

srun -N$NN -n$((NN*8)) -c7 --gpu-bind=closest python basic_usage.py \
--run_name $run_name --config $config --yaml_config $yaml_config --use_ddp >> log_pm25_wonorm_R2loss_newsplits 2>&1 &

export run_name="demo_vit_wonorm_R2loss_newsplits_rawgraph"
export yaml_config=./config/Demo_graph_vit_pm2.5_newsplits_rawgraph.yaml

srun -N$NN -n$((NN*8)) -c7 --gpu-bind=closest python basic_usage.py \
--run_name $run_name --config $config --yaml_config $yaml_config --use_ddp >> log_pm25_wonorm_R2loss_newsplits_rawgraph 2>&1 &
